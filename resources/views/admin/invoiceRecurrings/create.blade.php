...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.invoiceRecurring.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.invoice-recurrings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('billing_cycle') ? 'is-invalid' : '' }}" name="billing_cycle" id="billing_cycle" required>
                    ...
                    @foreach(App\Models\InvoiceRecurring::BILLING_CYCLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('billing_cycle', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
