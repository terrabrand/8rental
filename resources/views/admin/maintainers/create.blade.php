...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.maintainer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.maintainers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="user_id">{{ trans('cruds.maintainer.fields.user') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.maintainer.fields.user_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="email">{{ trans('cruds.maintainer.fields.email') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.maintainer.fields.email_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="password">{{ trans('cruds.maintainer.fields.password') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.maintainer.fields.password_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('units_assigneds') ? 'is-invalid' : '' }}" name="units_assigneds[]" id="units_assigneds" multiple>
                    @foreach($units_assigneds as $id => $units_assigned)
                        <option value="{{ $id }}" {{ in_array($id, old('units_assigneds', [])) ? 'selected' : '' }}>{{ $units_assigned }}</option>
                    @endforeach
                </select>
                @if($errors->has('units_assigneds'))
                    <div class="invalid-feedback">
                        {{ $errors->first('units_assigneds') }}
                    </div>
                @endif
