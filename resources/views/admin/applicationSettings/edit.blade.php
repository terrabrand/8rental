...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.applicationSetting.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.application-settings.update", [$applicationSetting->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="application_name">{{ trans('cruds.applicationSetting.fields.application_name') }}</label>
                <input class="form-control {{ $errors->has('application_name') ? 'is-invalid' : '' }}" type="text" name="application_name" id="application_name" value="{{ old('application_name', $applicationSetting->application_name) }}">
