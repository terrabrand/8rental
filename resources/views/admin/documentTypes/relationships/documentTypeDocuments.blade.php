...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-documentTypeDocuments">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($documents as $key => $document)
                        <tr data-entry-id="{{ $document->id }}">
                            ...
                            <td>
                                @foreach($document->document_types as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($document->tenants as $key => $item)
                                    <span class="badge badge-info">{{ $item->first_name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($document->landlords as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($document->properties as $key => $item)
                                    <span class="badge badge-info">{{ $item->property_name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($document->units as $key => $item)
...
@endcan
  ...
  let table = $('.datatable-documentTypeDocuments:not(.ajaxTable)').DataTable({ buttons: dtButtons })
